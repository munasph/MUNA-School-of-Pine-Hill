...
export class SetPasswordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.seo.update({ title: 'Set Password', description: 'Set your admin password.', path: '/set-password' });
  ...
  submit(): void {
    if (!this.token) {
      this.submitError = 'Invalid invite link.';
      return;
    }
    ...
    this.subs.add(
      this.authService.setPassword({
        ...
        error: (err: HttpErrorResponse) => {
          this.submitting = false;
          this.submitError = err.error?.message ?? 'Could not set password.';
        },
      }),
    );
  }
